...
function renderPrograms(c,lang) {
  const p=c.programs;
  return `${intro(p.title,p.intro,p.taglines)}<section class="page-shell"><div class="section-head program-guide"><h2>${esc(p.guideTitle)}</h2><p>${esc(p.guideBody)}</p></div><div class="program-list">${p.items.map((x,i)=>{const d=x[3]||p.pendingDetails;return `<details class="program-row" id="program-${i+1}"><summary><span class="program-number" aria-hidden="true">${String(i+1).padStart(2,'0')}</span><span class="program-overview"><span class="program-heading"><h2>${esc(x[0])}</h2>${pill(x[2])}</span><span class="program-description">${esc(x[1])}</span><span class="detail-prompt">${esc(p.detailPrompt)}</span></span></summary><div class="program-detail"><dl class="program-facts">${p.detailLabels.map((label,n)=>`<div class="program-fact"><dt>${esc(label)}</dt><dd>${esc(d[n])}</dd></div>`).join('')}</dl><div class="program-links">${i===0?link(lang,'practice',p.practiceLink):''}${link(lang,'contact',p.ask,'text-link',programTopics[i])}</div></div></details>`;}).join('')}</div><aside class="subactivity-note"><h2>${esc(p.subTitle)}</h2><p>${esc(p.subBody)}</p></aside></section>`;
}
